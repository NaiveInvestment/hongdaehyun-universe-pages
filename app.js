...
const INDICATOR_CATALOG = {
  ...
  "보험": [
    ...
    { name: "국고채 10년", source: "한은 ECOS 817Y002", cycle: "일", state: "fixed" },
    { name: "국고채 30년", source: "한은 ECOS 817Y002", cycle: "일", state: "fixed" },
    { name: "장단기 스프레드", source: "ECOS 817Y002 (30년 − 3년)", cycle: "일", state: "fixed" },
...
function rareCompareColumns() {
  ...
  return [
    { label: "시세, 수익률", columns: [
      { key: "price", label: "현재가", sub: "현지통화" },
      { key: "d1", label: "1D" }, { key: "ytd", label: "YTD" }, { key: "mdd", label: "MDD" },
    ] },
    { label: `기업가치 (${money})`, columns: [{ key: "cap", label: "시총 / 지분가치" }, { key: "ev", label: "EV", sub: "현재 기업가치" }] },
    ...RARE_FORWARD_YEARS.map(year => ({
      label: `${year} (${money})`, columns: [
        ...[["pe", "P/E"], ["evEbitda", "EV/EBITDA"]].map(([ratio, label]) => ({
          key: `${ratio}${year}`, ratio, ratioLabel: label, year, label, sortLabel: `${year} ${label}`,
        })),
        ...RARE_METRICS.map(([metric, label]) => ({
          key: `${metric}${year}`, metric, year, label: metric === "operatingIncome" ? "영업이익" : label,
          sub: metric === "operatingIncome" ? "EBIT" : "", sortLabel: `${year} ${label}`,
          financialStart: metric === "revenue",
        })),
      ],
    })),
  ];
...
function rareCompareCell(row, column, index) {
  ...
  let title = "", text = formatNumber(value, 1), extra = index === 0 ? "section-start" : column.financialStart ? "subsection-start" : "";
  if (column.metric) {
  ...
  } else if (column.key === "ev") {
    text = formatNumber(value, 0);
    const ev = company.valuation?.enterpriseValue;
    title = ev ? `TIKR 현재 기업가치, ${ev.asOf}, 원본 ${formatNumber(ev.value, 2)} ${ev.currency} 백만. 미래 순부채를 적용한 예상 EV가 아닙니다.` : "EV 미확보. 시총이나 비상장 지분가치를 EV로 대체하지 않습니다.";
...
function rareComparisonTable(packet) {
  ...
  return `<section id="rareValuationComparison" aria-labelledby="rareValuationTitle">
    <div class="toolbar re-compare-toolbar"><h2 id="rareValuationTitle">희토류 기업 비교</h2><span class="spacer"></span><span class="basis-switch" role="group" aria-label="표시 통화 선택">${currency}</span></div>
    <div class="re-compare-period"><span>시세, 수익률과 연도별 밸류에이션, 손익. 결산연도 2026–2028 전망, Lynas 2026은 실적</span><span class="spacer"></span><span>상장 ${rows.filter(r=>!r.company.private).length}개, 비상장 ${rows.filter(r=>r.company.private).length}개</span></div>
    <div id="tableScroller" class="table-region re-compare-region" role="region" tabindex="0" aria-label="희토류 시세와 2026~2028년 밸류에이션, 손익 비교, 가로 스크롤"><table id="universeTable" class="universe-table re-compare-table re-combined-table"><caption class="sr-only">희토류 기업별 시총 또는 최근 거래 지분가치, 2026~2028년별 P/E, EV/EBITDA, 매출, EBITDA, 영업이익/EBIT, 순이익. 배수는 x, 현재가는 현지통화, 나머지 금액은 ${state.rareCurrency === "KRW" ? "억원" : "USD 백만"}.</caption>
    ...
    <div class="re-compare-notes"><p>해외 시총과 EV 2026-09-04, 재무 TIKR 2026-09-07 조회. 국내 시총은 현재 시세, 전망은 ConsenDB ${HORIZON_LABELS[state.estimateBasis]}. 과거 손익은 결산기간 평균환율, 시총과 EV, 전망은 2026-09-04 환율입니다.</p><p>P/E = 시총 / 순이익, EV/EBITDA = 현재 EV / 해당 결산연도 TIKR EBITDA. 해외 순이익은 GAAP, 국내는 지배순이익 기준. N/M은 분모가 0 이하, -는 미제공. 국내와 비상장은 EV 미확보로 EV 배수를 표시하지 않습니다.</p><p>Lynas와 CRML은 6월 결산이며 Lynas 2026년은 실적입니다. 영업이익의 과거 값은 보고 영업이익입니다. EBITDA와 EBIT는 각각의 컨센서스로 차이가 감가상각과 일치하지 않을 수 있습니다. 비상장은 최근 거래 지분가치입니다.</p>
...
function rareCapModel(company, stock, fx) {
  ...
  const converted = rareMoneyValue(native, { target: state.rareCurrency, currency, unit: company.domestic ? "hundredMillion" : "millions", fx });
...
function rareFinancialTable(packet) {
  ...
  const fx = packet.fx;
  const metric = state.rareMetric;
  const target = state.rareCurrency;
  const unitLabel = target === "KRW" ? "KRW 억원" : "USD 백만";
  const targetFx = target === "KRW" ? fx?.krw : fx;
  const rateField = target === "KRW" ? "krwPerUnit" : "usdPerUnit";
  const currencyTabs = [["KRW", "원화"], ["USD", "USD"]].map(([key, text]) => `<button type="button" data-rare-currency="${key}" aria-pressed="${target === key}">${text}</button>`).join("");
  const label = RARE_METRICS.find(([key]) => key === metric)?.[1] || "매출";
  const displayNames = { "LYC.AX": "Lynas", EMAT: "EMAT", "NEO.TO": "Neo Performance" };
  const tabs = RARE_METRICS.map(([key, text]) => `<button type="button" data-rare-metric="${key}" aria-pressed="${metric === key}">${text}</button>`).join("");
  const companyBlocks = packet.companies.map(company => {
    ...
    const name = company.domestic ? `<a href="#/stock/${company.symbol}">${escapeHtml(displayName)}</a>`
      ...
      const cells = years.map(year => {
        ...
        const converted = rareMoneyValue(native, { target: state.rareCurrency, currency, unit: company.domestic ? "hundredMillion" : "millions", kind: raw?.kind, fiscalEnd, fx });
        ...
        const rateNote = currency === target ? `${target} 원본, 환산 없음` : converted.rate
          ? `${converted.basis}, ${converted.rate.periodStart}~${converted.rate.periodEnd}, 1 ${currency} = ${converted.rate[rateField].toPrecision(9)} ${target}`
          : "해당 환율 미확보";
        ...
        return `<td data-year="${year}" data-kind="${present ? kind : ""}" class="${kind === "E" ? "re-estimate" : ""} ${year === 2026 ? "re-forecast-edge" : ""}" title="${escapeHtml(title)}"><span class="re-number ${present ? "" : "na"}">${present ? formatNumber(converted.value, 1) : "-"}</span></td>`;
      }).join("");
  ...
  return `<section class="card re-panel re-financials" id="rareEarthFinancials" aria-labelledby="rareFinancialTitle">
    <header class="re-fin-heading"><div><h2 id="rareFinancialTitle">시가총액과 연간 ${escapeHtml(label)}</h2><strong class="re-unit">${unitLabel}</strong></div><div class="re-fin-actions"><div class="re-metric-tabs" role="group" aria-label="재무 지표 선택">${tabs}</div><div class="re-metric-tabs re-currency-tabs" role="group" aria-label="표시 통화 선택">${currencyTabs}</div></div></header>
    <div class="re-table-meta"><p>실적은 결산기간 평균환율, 시총과 전망은 <b>${escapeHtml(fx?.spot?.asOf || "미확인")}</b> 환율 고정</p><p>옅은 배경은 전망, - 미제공</p></div>
...
function updateLiveRow(stock) {
  ...
  if (peerCap) {
    const cap = rareCapModel({ domestic: true }, stock, state.snapshot?.rareEarth?.fx);
      peerCap.innerHTML = `<strong>${formatNumber(cap.value, 1)}</strong>`;
...
function bindEvents() {
  ...
  $("#view").addEventListener("click", (event) => {
    ...
    if (rareCurrency) {
      ...
      state.rareCurrency = rareCurrency.dataset.rareCurrency;
      renderView({ preserveScroll: true });
      $(".re-table-scroll").scrollLeft = scrollLeft;
      return $(`[data-rare-currency="${state.rareCurrency}"]`)?.focus({ preventScroll: true });
    }
    const rareMetric = event.target.closest("[data-rare-metric]");
    if (rareMetric) {
      const scrollLeft = $(".re-table-scroll")?.scrollLeft || 0;
      state.rareMetric = rareMetric.dataset.rareMetric;
      renderView({ preserveScroll: true });
      $(".re-table-scroll").scrollLeft = scrollLeft;
      return $(`[data-rare-metric="${state.rareMetric}"]`)?.focus({ preventScroll: true });
    }
